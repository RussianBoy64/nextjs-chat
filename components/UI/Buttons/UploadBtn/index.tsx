...
import inputStyles from "./uploadBtn.module.scss";
import { chatInputStore } from "@/store/chatInputStore";
import getNewPhotos from "helpers/getNewPhotos";
...
const UploadBtn = () => {
  const inpuFileRef: MutableRefObject<null | HTMLInputElement> = useRef(null);
  const [inputPhoto, addPhotos] = chatInputStore((state) => [
    state.inputPhotos,
    state.addPhotos,
  ]);
  const btnStyles = `${style.btn} ${style.btn_row}`;
  ...
  const onClickButtonHandler: MouseEventHandler<HTMLButtonElement> = () => {
    if (inpuFileRef.current) inpuFileRef.current.click();
  };

  return (
    <>
      <input
        className={inputStyles.uploadBtn__input}
        ref={inpuFileRef}
        onChange={onChangeInputHandler}
        type="file"
        accept={`${fileTypes.jpg},${fileTypes.png},${fileTypes.webp},`}
        multiple
      />
      <button
        className={btnStyles}
        onClick={onClickButtonHandler}
        title="Upload images(.webp, .png, .jpg)"
        type="button"
      >
        <svg
